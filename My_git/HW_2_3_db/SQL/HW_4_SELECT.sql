...
group by m.name, time_track

-- название альбомов, содержащих наименьшее количество треков.
select ma.name, count(mus_alboms_id) tracks from mus_alboms ma
join mus_tracks mt on mt.mus_alboms_id = ma.id
group by ma.name, mt.mus_alboms_id
having  count(mus_alboms_id) <= 1

select m.name, ma.name albom, mt.name song from musicians m
join links_alboms la on la.musicians_id = m.id 
join mus_alboms ma on ma.id = la.mus_alboms_id
join mus_tracks mt on mt.mus_alboms_id = ma.id 
group by m.name, ma.name, mt.name